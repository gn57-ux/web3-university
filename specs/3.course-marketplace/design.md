...
**实现阶段二次核对（`get_screen` + 截图）发现的更新**：

- Hero 实际是「短标题 + 长副文案」两段，而非上面摘要成的单句长标题：H1 为「发现 Web3 的未来」，下方副文案才是「掌握最新的区块链技术、智能合约开发和去中心化金融原理。开启您的数字主权之旅。」。实现以截图为准，采用两段式结构。
- 难度筛选控件在截图中呈单个「所有难度 ⌄」下拉样式，选项文案为「所有难度/入门/进阶/专家」；实现改为与全站一致的 Tab 按钮组，文案统一为「全部/初级/中级/高级」（复用 `lib/mock/courseLevel.ts` 的 `COURSE_LEVEL_LABEL`），避免同一门课程在筛选控件与卡片难度徽标上出现两套术语。视觉呈现（Tab 而非下拉）与截图有出入，但功能等价，属实现阶段的一致性取舍。
- 课程卡难度徽标位置为覆盖在封面图右上角（而非封面图下方的独立徽标行）。
